...
router.get("/", (0, middleware_1.authentication)(), (0, middleware_1.validation)(common_1.paginationValidationSchema), async (req, res, next) => {
...
router.patch("/:postId", (0, middleware_1.authentication)(), (0, multer_1.cloudFileUpload)({ validation: multer_1.fileFieldValidation.image }).array("attachments", 2), (0, middleware_1.validation)(validators.updatePost), async (req, res, next) => {
    const data = await post_service_1.postService.updatePost(req.params, { ...req.body, files: req.files }, req.user);
    return (0, common_1.successResponse)({ res, status: 200, data });
});
...
router.delete("/:postId", (0, middleware_1.authentication)(), (0, middleware_1.validation)(validators.deletePost), async (req, res, next) => {
    const data = await post_service_1.postService.deletePost(req.params, req.user, req.body.force);
    return (0, common_1.successResponse)({ res, status: 200, data });
});
router.put("/:postId/restore", (0, middleware_1.authentication)(), (0, middleware_1.validation)(validators.restorePost), async (req, res, next) => {
    const data = await post_service_1.postService.restorePost(req.params, req.user);
    return (0, common_1.successResponse)({ res, status: 200, data });
});
